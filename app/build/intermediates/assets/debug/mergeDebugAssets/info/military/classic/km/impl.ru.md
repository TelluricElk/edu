...
```kotlin
fun runKMeans(points: List<SupplyPoint>, k: Int, iterations: Int, seed: Int): KMeansResult {
    val rnd = Random(seed)
    var centroids = points.shuffled(rnd).take(k).map { it.requestSize to it.requestFrequency }.toMutableList()
    ...
    repeat(iterations) {
        ...
        centroids = (0 until k).map { j ->
            ...
            else cluster.map { it.requestSize }.average().toFloat() to cluster.map { it.requestFrequency }.average().toFloat()
        }.toMutableList()
    }
