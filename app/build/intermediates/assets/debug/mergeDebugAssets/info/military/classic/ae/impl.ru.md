...
Тот же принцип на Kotlin — переиспользуется тот же класс `Dense` (из `NeuralPrimitives.kt`), что и в теме «Полносвязная сеть», только теперь их четыре подряд, а функция потерь сравнивает выход не с меткой класса, а с самим входом:
