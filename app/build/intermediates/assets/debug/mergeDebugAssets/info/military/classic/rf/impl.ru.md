...
```kotlin
fun trainForest(data: List<CandidatePoint>, nTrees: Int, maxDepth: Int, criterion: DtCriterion): List<DtNodeMilitary> {
...
fun predictForest(trees: List<DtNodeMilitary>, point: CandidatePoint): Boolean {
    val votes = trees.map { DtLabMilitary.predict(it, point) }
