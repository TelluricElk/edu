...
class GridWorld:
    ...
    def step(self, pos, action_idx):
        ...
        if not (0 <= nx < self.size and 0 <= ny < self.size) or (nx, ny) in self.walls:
            return pos, -1.0, False              # врезался в ограждение/границу — остаётся на месте
        if (nx, ny) == self.goal:
...
Табличный Q-learning хранит отдельную оценку для каждой пары состояние-действие — это работает для маленьких дискретных пространств вроде полигона 8×8 (256 пар), но не масштабируется на большие или непрерывные пространства состояний (например, поток видео с камеры). Там на смену табличному Q приходит **Deep Q-Network (DQN)** — та же идея, но Q-значения предсказывает нейросеть, а не таблица.
