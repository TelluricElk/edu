...
Тот же алгоритм на Kotlin — цикл вперёд по последовательности, затем цикл назад для BPTT:

```kotlin
class RnnCell(hiddenSize: Int, seed: Int) {
    val wxh = FloatArray(hiddenSize) { /* ... */ }
    val whh = Array(hiddenSize) { FloatArray(hiddenSize) { /* ... */ } }
    val why = FloatArray(hiddenSize) { /* ... */ }

    fun forward(sequence: FloatArray): Pair<List<FloatArray>, Float> {
        // сохраняем h на КАЖДОМ шаге — понадобится для обратного прохода
    }

    fun backward(hHistory: List<FloatArray>, sequence: FloatArray, label: Float, lr: Float) {
        // идём от последнего шага к первому, накапливая градиент по общим весам
    }
}
```

Именно то, что веса `wxh`, `whh`, `why` — одни и те же объекты, используемые на каждом шаге цикла, и есть программное воплощение "разделяемых весов во времени" из раздела «Мат. основа».

### Важная оговорка

Учебная реализация — простейшая (Elman) RNN с одним скалярным входом на шаг. Настоящие LSTM и GRU устроены сложнее (несколько дополнительных "вентильных" весовых матриц на каждый шаг), но общий скелет — разворачивание во времени и BPTT — остаётся тем же самым.
