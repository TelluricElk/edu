...
Пример использования — тот же датасет "кандидат на курсы" (возраст, балл), что и в интерактиве:

```python
X_train = [[22, 78], [35, 60], [45, 40], ...]
y_train = ["зачислен", "зачислен", "отказано", ...]

tree = build_tree(np.array(X_train), np.array(y_train), max_depth=4)
predict_one(tree, [28, 82])
```
...
fun buildTree(data: List<CandidatePoint>, depth: Int, maxDepth: Int): TreeNode {
