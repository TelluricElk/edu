...
private val kmQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы k-средних: та же
 * логика, что у [KmResult] (который остаётся нетронутым и больше не
 * вызывается для id = "km"), только с примером сегментации точек
 * снабжения вместо покупателей.
 */
@Composable
fun KmResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFE63946)

    val state = remember { KmLabMilitary.run(k = 4, iterations = 15, seed = 7) }
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                AskChatButton(accent = accent, onClick = {
                    onOpenChat(
                        "Объясни, пожалуйста, простыми словами, почему получился именно такой результат в теме «${title ?: "K-средних"}» (Решение задачи).\n\n" +
                        "Параметры: k = 4, 15 итераций алгоритма Ллойда.\n" +
                        "Итоговая инерция: ${"%.0f".format(state.inertia)}\n\n" +
