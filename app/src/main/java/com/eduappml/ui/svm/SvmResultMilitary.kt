...
private val svmQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы SVM: та же логика, что
 * у [SvmResult] (который остаётся нетронутым и больше не вызывается для
 * id = "svm"), только с примером диагностики двигателя боевой машины.
 */
@Composable
fun SvmResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFB5179E)

    val model = remember { SvmLabMilitary.train(c = 1f, kernel = SvmKernel.LINEAR, gamma = 0.3f, iterations = 800) }
    val accuracy = remember { SvmLabMilitary.accuracy(model, SvmLabMilitary.testSet) }
    val svCount = remember { SvmLabMilitary.supportVectorCount(model) }
