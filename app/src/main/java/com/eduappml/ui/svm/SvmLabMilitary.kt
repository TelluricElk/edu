...
/**
 * Военный вариант "эталонной задачи" SVM: диагностика двигателя боевой
 * машины (исправно / предаварийное) по двум датчикам — температуре и
 * вибрации. Полный структурный аналог [SvmLab] (который остаётся
 * нетронутым и больше нигде не используется). Сама математика и структура
 * данных здесь уже были предметно-нейтральными (просто x1/x2/label), так
 * что меняется в первую очередь прикладной контекст в тексте, а не код.
 */
data class SvmPointMilitary(val x1: Float, val x2: Float, val label: Int) // label: -1 (предаварийное) или +1 (исправно)

object SvmLabMilitary {
    const val FEATURE_MIN = 0f
    const val FEATURE_MAX = 10f

    val trainSet: List<SvmPointMilitary> by lazy { generate(seed = 42, perClass = 26) }
    val testSet: List<SvmPointMilitary> by lazy { generate(seed = 777, perClass = 14) }

    private fun generate(seed: Int, perClass: Int): List<SvmPointMilitary> {
        val rnd = Random(seed)
        val points = mutableListOf<SvmPointMilitary>()
        repeat(perClass) {
            ...
            points.add(SvmPointMilitary(x1, x2, 1))
        }
        repeat(perClass) {
    ...
    private fun kernelValue(a: SvmPointMilitary, bx1: Float, bx2: Float, kernel: SvmKernel, gamma: Float): Float =
        when (kernel) {
    ...
    /** Обучение методом Pegasos — см. [SvmLab] для подробного комментария о bias. */
    ...
    fun accuracy(model: Model, data: List<SvmPointMilitary>): Float {
    ...
    /** Явные веса (w1, w2, bias) — доступны только для линейного ядра. */
