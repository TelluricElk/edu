...
/**
 * Военный вариант "эталонной задачи" GAN: имитация облака характеристик
 * сигнала генератором против дискриминатора, обученного отличать имитацию
 * от подлинника. Полный структурный аналог [GanLab] (который остаётся
 * нетронутым и больше нигде не используется) — только со сменой сцены
 * ("фальшивомонетчик/полицейский" → "имитация сигнала/станция обнаружения").
 * Переиспользует [Point2D] из [GanLab] (тот же пакет, класс уже
 * предметно-нейтрален — просто пара чисел (x, y)) и [Dense]/[Activation]
 * из общего пакета `common`.
 */
object GanLabMilitary {
    ...
    /** Фиксированный набор "настоящих" характеристик сигнала для отрисовки. */
    ...
    /** Средняя точность дискриминатора на равной смеси реальных и сгенерированных (0.5 = идеальный обман). */
