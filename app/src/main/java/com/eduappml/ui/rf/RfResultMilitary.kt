...
private val rfQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы случайного леса: та же
 * логика, что у [RfResult] (который остаётся нетронутым и больше не
 * вызывается для id = "rf"), только с примером зачисления кандидата.
 */
@Composable
fun RfResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFF9D4EDD)

    val forest = remember { RfLabMilitary.trainForest(nTrees = 25, maxDepth = 5) }
    val accuracy = remember { RfLabMilitary.accuracy(forest, RfLabMilitary.testSet) }
