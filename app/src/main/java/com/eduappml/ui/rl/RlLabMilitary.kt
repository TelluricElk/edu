...
/**
 * Военный вариант "эталонной задачи" Q-обучения: БПЛА учится находить путь
 * через полосу препятствий на тренировочном полигоне до пункта сбора.
 * Полный структурный аналог [RlLab] (который остаётся нетронутым и больше
 * нигде не используется) — математика, сетка и награды идентичны,
 * изменилась только сцена ("лабиринт" → "полигон с препятствиями").
 */
object RlLabMilitary {
    const val GRID = 8
    val START = 0 to 0
    val GOAL = 7 to 7

    /** Ограждение с одним проёмом на y=4 — заставляет агента найти обходной путь. */
