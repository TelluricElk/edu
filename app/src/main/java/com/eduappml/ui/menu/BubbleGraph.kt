...
import com.eduappml.ui.common.uiScale
...
/**
 * Карта-граф из «пузырей».
 *
 * Позиции узлов заданы долями холста (`xFrac`/`yFrac`) и поэтому тянутся за
 * размером экрана сами. А вот всё остальное раньше не тянулось:
 *  - радиусы пузырей были в фиксированных dp — на планшете холст вырастает
 *    вдвое, а пузыри остаются телефонными: редкие точки в пустоте;
 *  - толщина рёбер, импульсы и амплитуда дрейфа задавались СЫРЫМИ пикселями —
 *    то есть физически менялись от плотности экрана, а не от его размера.
 *
 * Теперь всё это умножается на [uiScale] от меньшей стороны холста, а пиксельные
 * константы пропущены через `designPx` (см. `ui/common/Adaptive.kt`). На эталонном
 * телефоне картинка не изменилась ни на пиксель — меняется только то, что раньше
 * ломалось на других устройствах.
 *
 * @param wallPadding «стенки», за которые пузырям нельзя заезжать. По умолчанию
 *        8dp со всех сторон — как было раньше (только сверху стенки не было
 *        вовсе, из-за чего в ландшафте верхние узлы уезжали под заголовок и
 *        статус-бар). Экраны передают сюда системные инсеты и высоту заголовка.
 */
...
fun BubbleGraph(
    ...
    wallPadding: PaddingValues = PaddingValues(8.dp)
) {
    BoxWithConstraints(modifier = modifier.fillMaxSize()) {
        ...
        val scale = uiScale(min(maxWidth, maxHeight))

        // Множитель для «исторических» пиксельных констант: приводит их к
        // физически одинаковому размеру на любой плотности и растит под планшет.
        val designUnit = density.density / Adaptive.ReferenceDensity * scale

        // --------- стенки ----------
        val wallLeftPx = with(density) { wallPadding.calculateLeftPadding(layoutDirection).toPx() }
        val wallRightPx = with(density) { wallPadding.calculateRightPadding(layoutDirection).toPx() }
        val wallTopPx = with(density) { wallPadding.calculateTopPadding().toPx() }
        val wallBottomPx = with(density) { wallPadding.calculateBottomPadding().toPx() }
        ...
        val driftParams = remember(nodes) {
            nodes.associate { n ->
                // Амплитуды остались прежними числами, но теперь это не пиксели,
                // а «эталонные пиксели»: ниже они умножаются на designUnit.
        ...
        fun rawDrift(id: String): Offset {
            ...
            return Offset(nx * p.ampX * designUnit, ny * p.ampY * designUnit)
        }
        ...
        // ---------- Жесты: TAP ----------
        val tapModifier = Modifier.pointerInput(nodes, scale) {
            detectTapGestures { pos ->
                // Зона нажатия не может быть меньше 44dp в диаметре: на узком
                // телефоне самый маленький пузырь — 24dp радиусом, но палец
                // попадает не в пиксель, а в подушечку. Рекомендация Material —
                // минимум 48dp на касание; берём 44dp как компромисс, чтобы
                // соседние пузыри не начали перехватывать чужие нажатия.
                val minHitPx = 22.dp.toPx()
                var hitId: String? = null
                var best = Float.MAX_VALUE
                nodes.forEach { n ->
                    val p = currentPositions[n.id] ?: return@forEach
                    val rPx = max(n.radiusDp.dp.toPx() * scale, minHitPx)
        ...
        Canvas(
            ...
            val repelRadiusPx = 110.dp.toPx() * scale
            ...
            // 3.5) «стенки» экрана.
            // Верхней стенки раньше не было вообще — обрабатывались только левая,
            // правая и нижняя. В портрете это не бросалось в глаза, а в ландшафте
            // и на планшете узлы с малым yFrac (AE, CNN, GAN) заезжали под
            // заголовок и под статус-бар.
            val wallStrength = 1.0f
            nodes.forEach { n ->
                val rPx = n.radiusDp.dp.toPx() * scale
                var p = positions[n.id] ?: return@forEach

                val leftOverlap = (wallLeftPx + rPx) - p.x
                if (leftOverlap > 0f) p = Offset(p.x + leftOverlap * wallStrength, p.y)

                val rightOverlap = p.x - (w - wallRightPx - rPx)
                if (rightOverlap > 0f) p = Offset(p.x - rightOverlap * wallStrength, p.y)

                val topOverlap = (wallTopPx + rPx) - p.y
                if (topOverlap > 0f) p = Offset(p.x, p.y + topOverlap * wallStrength)

                val bottomOverlap = p.y - (h - wallBottomPx - rPx)
            ...
            val maxStepPx = 24.dp.toPx() * scale * (dtSec / (1f / 60f))
            nodes.forEach { n ->
                ...
                if (step > maxStepPx && step > 0f) {
                    val scaleStep = maxStepPx / step
                    sm = Offset(prev.x + dx * scaleStep, prev.y + dy * scaleStep)
                }
            ...
            edges.forEach { e ->
                ...
                drawLine(
                    ...
                    start = pa, end = pb,
                    // Нижняя граница в 1 физический пиксель: полупрозрачная
                    // линия тоньше пикселя «размазывается» антиалиасингом и на
                    // пёстром фоне пропадает совсем. На современных экранах
                    // (плотность от 2.0) ограничение не срабатывает — там и без
                    // него выходит 1.5–2.5 пикселя.
                    strokeWidth = max(designPx(2f) * scale, 1f),
                    pathEffect = PathEffect.cornerPathEffect(designPx(36f))
                )
            }

            // --- ИМПУЛЬСЫ по рёбрам ---
            run {
                val speedPxPerSec = designPx(220f) * scale
                val pulsesPerEdge = 2
                val pulseRadiusBase = designPx(5f) * scale
                val pulseGlow = 1.8f
            ...
            nodes.forEach { n ->
                ...
                drawCircle(
                    ...
                    radius = rPx,
                    center = p,
                    style = Stroke(width = designPx(3f) * scale * alpha)
                )
                drawCircle(
                    color = Color.White.copy(alpha = 0.18f * alpha),
                    radius = max(0f, rPx - designPx(6f) * scale),
                    center = p
                )
