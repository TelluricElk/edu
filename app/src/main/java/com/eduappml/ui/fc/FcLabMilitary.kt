...
/**
 * Военный вариант "эталонной задачи" полносвязной сети: классификация
 * "Свой/Чужой" по показаниям двух датчиков, где правильный ответ зависит
 * от XOR-подобной комбинации сигналов, а не от каждого по отдельности.
 * Полный структурный аналог [FcLab] (который остаётся нетронутым и
 * больше нигде не используется) — только с военным прикладным контекстом
 * вместо абстрактного XOR-узора.
 */
data class SensorPoint(val signal1: Float, val signal2: Float, val label: Float) // label: 0f = "Чужой", 1f = "Свой"

object FcLabMilitary {
    const val FEATURE_MIN = 0f
    const val FEATURE_MAX = 10f

    /** XOR-узор: точки по диагонали (нижний левый + верхний правый) — "Свой". */
    val trainSet: List<SensorPoint> by lazy { generate(seed = 42, perCluster = 20) }
    val testSet: List<SensorPoint> by lazy { generate(seed = 777, perCluster = 10) }

    private fun generate(seed: Int, perCluster: Int): List<SensorPoint> {
        val rnd = Random(seed)
        val corners = listOf(
            Triple(2.5f, 2.5f, 1f),
            Triple(7.5f, 7.5f, 1f),
        ...
        corners.forEach { (cx, cy, label) ->
            repeat(perCluster) {
                val s1 = (cx + (rnd.nextFloat() - 0.5f) * 2.2f).coerceIn(FEATURE_MIN, FEATURE_MAX)
                val s2 = (cy + (rnd.nextFloat() - 0.5f) * 2.2f).coerceIn(FEATURE_MIN, FEATURE_MAX)
                points.add(SensorPoint(s1, s2, label))
            }
        }
        return points
    }
    ...
    /** Нормализация признаков в [-1, 1] — стандартный приём для устойчивого обучения (см. также LrLabMilitary). */
    private fun norm(x1: Float, x2: Float): Pair<Float, Float> =
        (x1 / FEATURE_MAX * 2f - 1f) to (x2 / FEATURE_MAX * 2f - 1f)

    /**
     * Маленькая нейросеть: 2 входа (два датчика) -> [hiddenSize] нейронов ReLU -> 1 выход (сигмоида).
    ...
    class Network(val hiddenSize: Int, seed: Int = 7) {
        ...
        fun trainStep(data: List<SensorPoint>, lr: Float) {
            ...
            data.forEach { p ->
                val (x1, x2) = norm(p.signal1, p.signal2)
                val (z1, h, pred) = forward(p.signal1, p.signal2)
    ...
    fun loss(net: Network, data: List<SensorPoint>): Float {
        if (data.isEmpty()) return 0f
        var sum = 0.0
        data.forEach { p ->
            val pred = net.predict(p.signal1, p.signal2).coerceIn(1e-6f, 1f - 1e-6f)
            sum += -(p.label * ln(pred.toDouble()) + (1 - p.label) * ln(1.0 - pred.toDouble()))
        }
        return (sum / data.size).toFloat()
    }

    fun accuracy(net: Network, data: List<SensorPoint>): Float {
        if (data.isEmpty()) return 0f
        val correct = data.count { (net.predict(it.signal1, it.signal2) >= 0.5f) == (it.label >= 0.5f) }
