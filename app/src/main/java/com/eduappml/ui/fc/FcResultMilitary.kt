...
private val fcQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы полносвязной сети: та
 * же логика, что у [FcResult] (который остаётся нетронутым и больше не
 * вызывается для id = "fc"), только с примером слияния двух датчиков
 * вместо абстрактного XOR-узора.
 */
@Composable
fun FcResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFFF6B6B)

    val result = remember { FcLabMilitary.train(hiddenSize = 6, lr = 0.5f, epochs = 300) }
    val accuracy = remember { FcLabMilitary.accuracy(result.network, FcLabMilitary.testSet) }
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                AskChatButton(accent = accent, onClick = {
                    onOpenChat(
                        "Объясни, пожалуйста, простыми словами, почему получился именно такой результат в теме «${title ?: "Полносвязная сеть"}» (Решение задачи).\n\n" +
                        "Параметры: 6 нейронов в скрытом слое, 300 эпох обучения.\n" +
                        "Точность на контрольной выборке: ${(accuracy * 100).roundToInt()}%\n\n" +
