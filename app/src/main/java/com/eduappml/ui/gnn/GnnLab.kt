...
const val NODE_COUNT = 14
const val FEAT_DIM = 2

object GnnLab {
    /** Список смежности — фиксированный граф с двумя сообществами (первые 7 узлов и последние 7). */
    val adjacency: List<MutableList<Int>> by lazy { buildGraph(seed = 42) }
    val community: IntArray by lazy { IntArray(NODE_COUNT) { if (it < NODE_COUNT / 2) 0 else 1 } }
    val features: Array<FloatArray> by lazy { generateFeatures(seed = 1) }

    private fun buildGraph(seed: Int): List<MutableList<Int>> {
        val rnd = Random(seed)
        val adj = List(NODE_COUNT) { mutableListOf<Int>() }
        val half = NODE_COUNT / 2
        for (i in 0 until NODE_COUNT) {
            for (j in i + 1 until NODE_COUNT) {
                val sameCommunity = (i < half) == (j < half)
                val p = if (sameCommunity) 0.6f else 0.05f
                if (rnd.nextFloat() < p) {
                    adj[i].add(j)
                    adj[j].add(i)
                }
    ...
    private fun generateFeatures(seed: Int): Array<FloatArray> {
        val rnd = Random(seed)
        // Признаки НАМЕРЕННО случайны и не связаны с сообществом — узнать сообщество
        // можно только через структуру связей, а не через сами эти числа.
        return Array(NODE_COUNT) { FloatArray(FEAT_DIM) { (rnd.nextFloat() - 0.5f) } }
    }
    ...
    fun train(numLayers: Int, hidden: Int, lr: Float, epochs: Int, seed: Int = 11): Model {
        val layers = (0 until numLayers).map { i ->
            GnnLayer(if (i == 0) FEAT_DIM else hidden, hidden, seed + i * 7)
        }
    ...
    fun accuracy(model: Model): Float {
        ...
        return correct.toFloat() / NODE_COUNT
    }
}
