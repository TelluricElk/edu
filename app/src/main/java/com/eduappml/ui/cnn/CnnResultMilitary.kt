...
private val cnnQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы CNN: та же логика, что
 * у [CnnResult] (который остаётся нетронутым и больше не вызывается для
 * id = "cnn"), только с примером ориентации линейного объекта на
 * аэрофотоснимке вместо абстрактной линии.
 */
@Composable
fun CnnResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFFFD93D)
    val topicTitle = title ?: "Свёрточная сеть"

    val net = remember { CnnLabMilitary.train(nFilters = 4, lr = 0.05f, epochs = 20) }
    val accuracy = remember { CnnLabMilitary.accuracy(net, CnnLabMilitary.testSet) }
    ...
    LessonScaffold(
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text(
                    "Точность на контрольной выборке (другие позиции объекта, чем при обучении): ${(accuracy * 100).roundToInt()}%",
