...
private val trQuiz = listOf(
    ...
    QuizQuestion(
        ...
        "Сам механизм self-attention симметричен относительно перестановки слов — без positional encoding «кот съел мышь» и «мышь съела кота» выглядели бы для модели одинаково."
    ),
    QuizQuestion(
    ...
    LessonScaffold(
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Text(
                    "Слова «кот» и «мышь» — оба существительные, оба одушевлённые в наших вручную заданных признаках.",
        ...
        QuizSection(questions = trQuiz, textColor = textColor, nodeId = "tr")
    }
}
