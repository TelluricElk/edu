...
private val aeQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы автокодировщика: та же
 * логика, что у [AeResult] (который остаётся нетронутым и больше не
 * вызывается для id = "ae"), только с примером сжатия маршрута
 * патрулирования БПЛА вместо абстрактной дуги.
 */
@Composable
fun AeResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFFF914D)
    val topicTitle = title ?: "Автокодировщик"

    val ae = remember { AeLabMilitary.train(bottleneck = 1, lr = 0.15f, epochs = 300) }
    val mse = remember { AeLabMilitary.mse(ae, AeLabMilitary.testSet) }
    ...
    LessonScaffold(
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(4.dp))
                Text(
                    "Даже сжимая пару чисел (X, Y) до одного, сеть восстанавливает положение засечки на маршруте с небольшой ошибкой — потому что маршрут по сути одномерен.",
