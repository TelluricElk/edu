...
private val nbQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы наивного Байеса: та же
 * логика, что у [NbResult] (который остаётся нетронутым и больше не
 * вызывается для id = "nb"), только с примером "занятия в поле или в
 * помещении" вместо "пляж или дом".
 */
@Composable
fun NbResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFF4D96FF)

    val stats = remember { NbLabMilitary.fitStats(NbLabMilitary.trainSet) }
    val accuracy = remember { NbLabMilitary.accuracy(stats, NbLabMilitary.testSet) }
