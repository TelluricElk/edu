...
private val gbQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы градиентного бустинга:
 * та же логика, что у [GbResult] (который остаётся нетронутым и больше не
 * вызывается для id = "gb"), только с примером расхода топлива вместо
 * цены квартиры.
 */
@Composable
fun GbResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFF00C2A8)

    val model = remember { GbLabMilitary.train(nEstimators = 40, learningRate = 0.15f) }
