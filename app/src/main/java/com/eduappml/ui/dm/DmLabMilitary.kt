...
/**
 * Военный вариант "эталонной задачи" диффузионной модели: восстановление
 * положений двух опорных объектов на снимке из шума помех канала передачи.
 * Полный структурный аналог [DmLab] (который остаётся нетронутым и больше
 * нигде не используется) — только со сменой сцены ("старая фотография" →
 * "аэрофотоснимок, повреждённый помехами"). Переиспользует [Point2D] из
 * [DmLab] (тот же пакет, класс уже предметно-нейтрален).
 */
object DmLabMilitary {
