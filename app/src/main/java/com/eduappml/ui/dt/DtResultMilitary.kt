...
private val dtQuizMilitary = listOf(
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы дерева решений: та же
 * логика, что у [DtResult] (который остаётся нетронутым и больше не
 ...
 */
@Composable
fun DtResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFFF914D)

    val tree = remember { DtLabMilitary.buildTree(DtLabMilitary.trainSet, DtCriterion.GINI, maxDepth = 4, minSamplesSplit = 4) }
    val trainAcc = remember { DtLabMilitary.accuracy(tree, DtLabMilitary.trainSet) }
    val testAcc = remember { DtLabMilitary.accuracy(tree, DtLabMilitary.testSet) }
    ...
    LessonScaffold(
        ...
        QuizSection(questions = dtQuizMilitary, textColor = textColor, nodeId = "dt")
    }
}
