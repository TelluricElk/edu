...
/**
 * Военный вариант "эталонной задачи" SOM: организация складских красок для
 * маскировочной окраски по цветовому сходству. Полный структурный аналог
 * [SomLab] (который остаётся нетронутым и больше нигде не используется) —
 * математика и данные (случайные точки в RGB) идентичны, изменилась только
 * сцена ("коробка с красками" → "склад красок для маскировки").
 */
data class PaintColor(val r: Float, val g: Float, val b: Float)

object SomLabMilitary {
    ...
    val inputColors: List<PaintColor> by lazy { generateColors(seed = 42, count = COLOR_COUNT) }

    private fun generateColors(seed: Int, count: Int): List<PaintColor> {
        val rnd = Random(seed)
        return List(count) { PaintColor(rnd.nextFloat(), rnd.nextFloat(), rnd.nextFloat()) }
    }
    ...
    class Map(seed: Int) {
        ...
        fun findBmu(x: PaintColor): Pair<Int, Int> {
            var bestGx = 0
            var bestGy = 0
        ...
        fun trainStep(input: PaintColor, t: Int, decayHorizon: Int) {
