...
data class RnnSequence(val signals: List<Float>, val label: Float) // label: 1f = включён, 0f = выключен

object RnnLab {
    const val HIDDEN = 4

    /** Датасет "выключатель" генерируется заново под конкретную длину — длина сама по себе ключевой параметр темы. */
    fun trainSet(length: Int): List<RnnSequence> = generate(seed = 42, n = 50, length = length)
    fun testSet(length: Int): List<RnnSequence> = generate(seed = 777, n = 20, length = length)

    private fun generate(seed: Int, n: Int, length: Int): List<RnnSequence> {
        val rnd = Random(seed)
        return List(n) {
            val signals = List(length) { rnd.nextInt(2).toFloat() }
            var state = 0f
            signals.forEach { s -> if (s > 0.5f) state = 1f - state }
            RnnSequence(signals, state)
        }
    }
    ...
    fun train(length: Int, lr: Float, epochs: Int, seed: Int = 3): Network {
        ...
        repeat(epochs) {
            data.forEach { seq -> net.trainStep(seq.signals, seq.label, lr) }
        }
        return net
    }

    fun accuracy(net: Network, data: List<RnnSequence>): Float {
        if (data.isEmpty()) return 0f
        val correct = data.count { (net.predict(it.signals) >= 0.5f) == (it.label >= 0.5f) }
