...
private val rnnQuizMilitary = listOf(
    QuizQuestion(
    ...
    QuizQuestion(
        "Почему задачу с состоянием маяка нельзя решить сетью вроде CNN, смотрящей только на локальное окошко?",
        listOf(
            QuizOption("Финальное состояние зависит от ВСЕХ импульсов последовательности сразу, а не от каких-то нескольких соседних", true),
        ...
        "Чётность (или, как здесь, финальное состояние маяка) — классический пример признака, требующего информации о произвольно далёком прошлом, а не о локальной окрестности."
    ),
    QuizQuestion(
...
/**
 * Военный вариант экрана "Решение задачи" для темы RNN: та же логика, что
 * у [RnnResult] (который остаётся нетронутым и больше не вызывается для
 * id = "rnn"), только со сценой радиомаяка вместо выключателя света.
 */
@Composable
fun RnnResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFF6BCB77)
    val topicTitle = title ?: "Рекуррентная сеть"

    val shortNet = remember { RnnLabMilitary.train(length = 3, lr = 0.15f, epochs = 300) }
    val shortAcc = remember { RnnLabMilitary.accuracy(shortNet, RnnLabMilitary.testSet(3)) }
    val longNet = remember { RnnLabMilitary.train(length = 8, lr = 0.15f, epochs = 300) }
    val longAcc = remember { RnnLabMilitary.accuracy(longNet, RnnLabMilitary.testSet(8)) }
    ...
    LessonScaffold(
                ...
                AskChatButton(accent = accent, onClick = {
                    onOpenChat(
                        buildResultChatPrompt(
                            ...
                            "точность при длине 3 — ${(shortAcc * 100).roundToInt()}%, при длине 8 — ${(longAcc * 100).roundToInt()}%"
                        )
                    )
                })
            }
        ...
        Card(modifier = Modifier.fillMaxWidth(), colors = CardDefaults.cardColors(containerColor = Color.White.copy(alpha = 0.08f))) {
            Column(modifier = Modifier.padding(16.dp)) {
                ...
                Spacer(Modifier.height(8.dp))
                listOf(
                    "RNN хранит скрытое состояние — «память» обо всей последовательности до текущего момента.",
                    "Одни и те же веса используются на каждом шаге времени — разделяемые веса, аналог CNN во времени.",
                    "Обучение (BPTT) разворачивает сеть по времени и суммирует градиент со всех шагов.",
                    "Затухающий градиент — структурная причина, по которой простая RNN плохо помнит начало длинных последовательностей."
                ).forEach { Text("•  $it", color = textColor.copy(alpha = 0.85f), fontSize = 14.sp, modifier = Modifier.padding(vertical = 2.dp)) }
            }
        }

        Spacer(Modifier.height(20.dp))
        QuizSection(questions = rnnQuizMilitary, textColor = textColor, nodeId = "rnn")
    }
}
