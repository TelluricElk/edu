...
private val logrQuizMilitary = listOf(
    QuizQuestion(
    ...
    QuizQuestion(
        ...
        listOf(
            QuizOption("Когда недооценить готовность (например, преждевременно посчитать бойца готовым) дороже, чем перестраховаться", true),
...
/**
 * Военный вариант экрана "Решение задачи" для темы логистической регрессии:
 * та же логика, что у [LogrResult] (который остаётся нетронутым и больше
 * не вызывается для id = "logr"), только с примером сдачи норматива по
 * физподготовке вместо сдачи экзамена студентом.
 */
@Composable
fun LogrResultMilitary(modifier: Modifier = Modifier, title: String?, onBack: () -> Unit, onOpenChat: (String) -> Unit = {}) {
    val textColor = Color.White
    val accent = Color(0xFFFFD93D)

    val fit = remember { LogrLabMilitary.fit(0.3f, 300) }
    val cm = remember { LogrLabMilitary.confusionMatrix(LogrLabMilitary.testSet, fit.w, fit.b, 0.5f) }
