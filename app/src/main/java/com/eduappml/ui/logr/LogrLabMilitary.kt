...
/**
 * Военный вариант "эталонной задачи" логистической регрессии: предсказание
 * вероятности сдачи норматива по физподготовке по числу часов тренировок.
 * Полный структурный аналог [LogrLab] (который остаётся нетронутым и
 * больше нигде не используется) — только с военным прикладным контекстом
 * вместо примера со студентом и экзаменом.
 */
data class TrainingPoint(val hours: Float, val passed: Float) // passed: 0f или 1f

object LogrLabMilitary {
    const val HOURS_MIN = 0f
    const val HOURS_MAX = 12f

    val trainSet: List<TrainingPoint> by lazy { generate(seed = 42, n = 50) }
    val testSet: List<TrainingPoint> by lazy { generate(seed = 777, n = 20) }

    private fun generate(seed: Int, n: Int): List<TrainingPoint> {
        val rnd = Random(seed)
        return (0 until n).map {
            val hours = HOURS_MIN + rnd.nextFloat() * (HOURS_MAX - HOURS_MIN)
            // истинная вероятность сдачи растёт с часами тренировок (порог около 6 часов)
            val trueProb = sigmoid(1.1f * (hours - 6f))
            val passed = if (rnd.nextFloat() < trueProb) 1f else 0f
            TrainingPoint(hours, passed)
        }
    }

    fun sigmoid(z: Float): Float = (1.0 / (1.0 + exp(-z.toDouble()))).toFloat()

    private fun gradientStep(data: List<TrainingPoint>, w: Float, b: Float, lr: Float): Pair<Float, Float> {
        var gradW = 0f
        var gradB = 0f
        data.forEach { p ->
    ...
    fun confusionMatrix(data: List<TrainingPoint>, w: Float, b: Float, threshold: Float): ConfusionMatrix {
